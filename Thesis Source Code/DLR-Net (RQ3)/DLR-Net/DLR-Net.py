...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, output_stride, BatchNorm, pretrained=True):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.renet1 = ReNet(64, 32)
        self.renet2 = ReNet(32 * 2, 32)
        self.bn11 = BatchNorm(64)
        self.renet3 = ReNet(256, 128)
        self.renet4 = ReNet(128 * 2, 128)
        self.bn22 = BatchNorm(256)
        self.renet5 = ReNet(512, 256)
        self.renet6 = ReNet(256 * 2, 256)
        self.bn33 = BatchNorm(512)
...
class ASPP(nn.Module):
    def __init__(self, backbone, output_stride, BatchNorm):
        ...
        self.aspp1 = _ASPPModule(inplanes, 512, 1, padding=0, dilation=dilations[0], BatchNorm=BatchNorm)
        self.aspp2 = _ASPPModule(inplanes, 512, 3, padding=dilations[1], dilation=dilations[1], BatchNorm=BatchNorm)
        self.aspp3 = _ASPPModule(inplanes, 512, 3, padding=dilations[2], dilation=dilations[2], BatchNorm=BatchNorm)
        self.aspp4 = _ASPPModule(inplanes, 512, 3, padding=dilations[3], dilation=dilations[3], BatchNorm=BatchNorm)

        self.global_avg_pool = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)),
                                             nn.Conv2d(inplanes, 512, 1, stride=1, bias=False),
                                             BatchNorm(512),
                                             nn.ReLU())
        self.conv1 = nn.Conv2d(2560, 512, 1, bias=False)
...
class Decoder(nn.Module):
    def __init__(self, num_classes, backbone, BatchNorm):
        ...
        self.conv2 = nn.Conv2d(512, 96, 1, bias=False)
        self.bn2 = BatchNorm(96)
        self.relu = nn.ReLU()
        self.l2_conv = nn.Sequential(nn.Conv2d(1024, 512, kernel_size=3, stride=1, padding=1, bias=False),
                                       BatchNorm(512),
                                       nn.ReLU(),
                                       nn.Dropout(0.5),
        ...
        self.relu = nn.ReLU()
        self.l1_conv = nn.Sequential(nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                       BatchNorm(256),
                                       nn.ReLU(),
                                       nn.Dropout(0.3),
        ...
        self.convo = nn.Conv2d(64, 24, 1, bias=False)
        self.bno = BatchNorm(24)
        self.relu = nn.ReLU()
        self.lo_conv = nn.Sequential(nn.Conv2d(128, 64, kernel_size=3, stride=1, padding=1, bias=False),
                                       BatchNorm(64),
                                       nn.ReLU(),
                                       nn.Dropout(0.1),
    ...
    def forward(self, x, xo, x1, x2):
        #x2 = self.conv2(x2)
        #x2 = self.bn2(x2)
        #x2 = self.relu(x2)
        x = F.interpolate(x, size=x2.size()[2:], mode='bilinear', align_corners=True)
        #print("should be 512x32x64")
        #print(x.size())
        x = torch.cat((x, x2), dim=1)
        #print("should be 608x32x64")
        #print(x.size())
        x = self.l2_conv(x)
        #print("should be 256x32x64")
        #print(x.size())
        ...
        #print("should be 256x64x128")
        #print(x.size())
        x = torch.cat((x, x1), dim=1)
        #print("should be 304x64x128")
        #print(x.size())
        x = self.l1_conv(x)
        #print("should be 64x64x128")
        #print(x.size())
        
        #xo = self.convo(xo)
        #xo = self.bno(xo)
        #xo = self.relu(xo)
        x = F.interpolate(x, size=xo.size()[2:], mode='bilinear', align_corners=True)
        #print("should be 64x128x256")
        #print(x.size())
        x = torch.cat((x, xo), dim=1)
        #print("should be 88x128x256")
        #print(x.size())
        x = self.lo_conv(x)
        #print("should be 35x128x256")
        #print(x.size())
...
class DeepLab(nn.Module):
    ...
    def forward(self, input):
        x, xo ,x1, x2 = self.backbone(input)
        #print("before Aspp")
        #print(x.size())
        x = self.aspp(x)
        #print("After Aspp")
        #print(x.size())
        x = self.decoder(x, xo ,x1, x2)
        #print("output of decoder")
        #print(x.size())
        x = F.interpolate(x, size=input.size()[2:], mode='bilinear', align_corners=True)
        #print("after final interpolate")
        #print(x.size())
...
opt1 = optim.SGD(model.parameters(), lr=1e-6, momentum=0.7,nesterov=True)
...
import os
path2models= "./Deeplabv3+mod_CRNNBNReLuonly07"
...
torch.save(model.state_dict(), "./Deeplabv3+mod_CRNNBNReLuonly07_m")
...
model.load_state_dict(torch.load("./Deeplabv3+mod_CRNNBNReLuonly07_m"))
model.eval()
...
plt.imshow(color_img(pred[0].cpu()))
plt.savefig('DLR-Net1.png', dpi = 300)
...
plt.imshow(color_img(pred[1].cpu()))
plt.savefig('DLR-Net2.png', dpi = 300)
...
plt.imshow(color_img(pred[2].cpu()))
plt.savefig('DLR-Net3.png', dpi = 300)
...
plt.imshow(color_img(pred[3].cpu()))
plt.savefig('DLR-Net4.png', dpi = 300)
...
plt.imshow(color_img(pred[4].cpu()))
plt.savefig('DLR-Net5.png', dpi = 300)
